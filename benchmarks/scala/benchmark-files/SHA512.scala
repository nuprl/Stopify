...
class SHA512Context(val is384: Boolean) {
  ...
  def process(data: Array[Byte], start: Int): Unit = {
    ...
    @inline def P(a: Long, b: Long, c: Long, d: Long, e: Long, f: Long, g: Long,
        h: Long, x: Long, K: Long): (Long, Long) = {
    ...
    do {
      {
        ...
        val (newD, newH) = P(A, B, C, D, E, F, G, H, W(i), K(i)); D = newD; H = newH; i += 1
        val (newC, newG) = P(H, A, B, C, D, E, F, G, W(i), K(i)); C = newC; G = newG; i += 1
        val (newB, newF) = P(G, H, A, B, C, D, E, F, W(i), K(i)); B = newB; F = newF; i += 1
        val (newA, newE) = P(F, G, H, A, B, C, D, E, W(i), K(i)); A = newA; E = newE; i += 1
      }

      {
        // scoping the newABC variables
        val (newH, newD) = P(E, F, G, H, A, B, C, D, W(i), K(i)); H = newH; D = newD; i += 1
        val (newG, newC) = P(D, E, F, G, H, A, B, C, W(i), K(i)); G = newG; C = newC; i += 1
        val (newF, newB) = P(C, D, E, F, G, H, A, B, W(i), K(i)); F = newF; B = newB; i += 1
        val (newE, newA) = P(B, C, D, E, F, G, H, A, W(i), K(i)); E = newE; A = newA; i += 1
      }
    } while (i < 80)
