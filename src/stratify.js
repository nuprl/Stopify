/** Module to stratify a simplified javascript AST. Under our assumptions,
 * programs can only diverge at function call-sites. We name every function
 * application to ease the implementation of further transforming ASTs to
 * insert checkpoints at each application.
 *
 * We stratify an AST `e` into terms `m` that _must_ terminate, and terms `p`
 * that are _possibly non-termination_.
 *
 * Must Terminate m ::= c | x | m_1 op m_2 | if (m_1) { m_2 } else { m_3 } |
 * function(x,...) { p } | let x = m_1; m_2
 *
 * Possibly Nonterminating p ::= m | let x = m; p | let x = m_1(m_2,...); p |
 * if (m_1) { p_1 } else { p_2 }
 *
 * TODO(rachit & sam): This doesn't need to be recursive, we can just modify
 * anf.js to do a minimal anf transformation taking termination into
 * consideration.
 */
...
// stratifyExpression takes an Expression node, a continuation, and a path
// argument.
//
// It returns a pair of a _must_ terminate expression and a list of statements.
// These statements are potential variable declarations that must be prepended
// to the expression's parent statement, which is inaccessible via `path` in
...
//
// It returns an array of stratified statements, including any new variable
// declarations required to stratify the statement argument.
...
visitor.Program = function (path) {
  ...
  // map over each statement in our program. stratifying each statement
  // produces an array of statements, so we need to flatten this array of
