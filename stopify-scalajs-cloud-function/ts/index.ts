...
const scalajsc = path.resolve(__dirname, '../dist/bin/scalajsc');

export function stopifyCompileScalaJS(req: Request, resp: Response) {
  const code = req.body.toString();
  return tmpDir().then(dir => {
    const scalaSrcPath = `${dir}/Main.scala`;
    return fs.writeFile(scalaSrcPath, code)
      .then(() => exec(`scalajsc -d . ${scalaSrcPath}`, dir))
      // NOTE(arjun):  scalajsld returns exit code 0 even if the arguments are
      // wrong!
      .then(() => exec(`scalajsld -o main.js -mm Runner.main . --sourceMap .`, dir))
