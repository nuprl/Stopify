...
interface RuntimeOpts {
  estimator?: "reservoir" | "exact" | "countdown",
...
By default, Stopify uses the \texttt{reservoir} estimator that samples the current
time (using \lstinline|Date.now()|) and tries to yield every 100 milliseconds.
This mechanism is inexact, but performs well. You can adjust the yield
interval, but we do not recommend using a value lower than 100.
...
the \texttt{reservoir} estimator is nondeterministic.
...
overhead than \texttt{reservoir} and we do not recommend it.
